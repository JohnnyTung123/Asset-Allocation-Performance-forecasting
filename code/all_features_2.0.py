...
all_features_2.0.py
-------------------
v2 improvements over all_features.py:

  1. Fold-model ensemble — test predictions are the average of all 8 CV fold
     models instead of a single model re-trained on all data.
  2. Binary cross-entropy objective — better aligned with the sign-prediction
     task than MSE; LightGBM outputs calibrated probabilities, threshold at 0.5.
  3. num_boost_round=2000 + early stopping (50 rounds patience) — prevents
     under- and over-training; best iteration per fold is recorded and the mean
     is used for the final ensemble reference model.
  4. Regularised hyperparameters — num_leaves, min_child_samples,
     feature_fraction, bagging_fraction, lambda_l1 added.
  5. Per-GROUP target encoding (GROUP_ENC_TARGET) — coarser but more stable
     complement to the per-ALLOCATION encoding (ALLOC_ENC).

Public API (unchanged from v1):
    build_features(df, ret_cols, vol_cols) -> (df, feature_names)
    apply_target_encoding(df_fit, y_fit, df_transform) -> df_transform
        Now also adds GROUP_ENC_TARGET alongside ALLOC_ENC.
"""
...
def build_features(df, ret_cols, vol_cols):
    """Build benchmark + all engineered features (excluding ALLOC_ENC / GROUP_ENC_TARGET).

    Returns (df_with_features, feature_names).
    Call on X_train and X_test before the CV loop.
    Target encodings are excluded here — use apply_target_encoding() inside each fold.
    """
    all_features = []
...
def apply_target_encoding(df_fit, y_fit, df_transform, smoothing=10):
    """Smoothed target encoding for ALLOCATION and GROUP.

    v2: now encodes both ALLOCATION (-> ALLOC_ENC) and GROUP (-> GROUP_ENC_TARGET)
    in a single call. Both are fit on df_fit/y_fit and applied to df_transform.

    Must be called inside each CV fold to avoid label leakage:
    ...
    for col, enc_name in [('ALLOCATION', 'ALLOC_ENC'), ('GROUP', 'GROUP_ENC_TARGET')]:
        stats   = y_fit.groupby(df_fit[col]).agg(['mean', 'count'])
        encoded = (
            (stats['count'] * stats['mean'] + smoothing * global_mean)
            / (stats['count'] + smoothing)
        )
        out[enc_name] = out[col].map(encoded).fillna(global_mean)

    return out


# ---------------------------------------------------------------------------
# Full pipeline (run directly: python all_features_2.0.py)
# ---------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    LGBM_PARAMS = {
        'objective':          'binary',       # v2: binary CE instead of MSE
        'metric':             'binary_logloss',
        'num_threads':        50,
        'seed':               42,
        'verbosity':          -1,
        'learning_rate':      1e-2,
        # v2: regularised hyperparameters
        'num_leaves':         31,
    ...
    NUM_BOOST_ROUND  = 2000   # v2: increased; early stopping prevents over-training
    EARLY_STOPPING   = 50
    N_SPLITS         = 8
    ...
    if USE_TARGET_ENCODING:
        features = features + ['ALLOC_ENC', 'GROUP_ENC_TARGET']   # v2: +GROUP encoding

    print(f"Total features: {len(features)}")

    # --- 8-fold CV (split on dates, not rows) ---
    train_dates = X_train['TS'].unique()
    scores, models, best_iters = [], [], []
    ...
    for fold, (tr_idx, val_idx) in enumerate(splits):
        ...
        # v2: binary objective requires 0/1 labels
        y_tr_label  = (y_tr  > 0).astype(int).values
        y_val_label = (y_val > 0).astype(int).values
        ...
        # v2: early stopping against the validation set
        model = lgbm.train(
            LGBM_PARAMS,
            lgbm.Dataset(X_tr,  label=y_tr_label),
            num_boost_round=NUM_BOOST_ROUND,
            valid_sets=[lgbm.Dataset(X_val, label=y_val_label)],
            callbacks=[lgbm.early_stopping(EARLY_STOPPING), lgbm.log_evaluation(0)],
        )

        # v2: threshold at 0.5 (probabilities) instead of 0 (raw scores)
        preds = model.predict(X_val.values, num_threads=LGBM_PARAMS['num_threads'])
        acc   = accuracy_score(y_val_label, (preds > 0.5).astype(int))
    ...
    print(f"Mean best iteration across folds: {int(np.mean(best_iters))}")
    ...
    plt.title("Top-30 features by mean gain (8-fold) — v2")
    plt.tight_layout()
    plt.savefig('feature_importance_v2.png', dpi=150)
    plt.show()

    # --- Submission: ensemble of fold models (v2 improvement #1) ---
    # Average predictions from all 8 CV models instead of training a fresh final model.
    # Each fold model was trained on ~7/8 of the data with its own best stopping point.
    if USE_TARGET_ENCODING:
        X_test_enc = apply_target_encoding(X_train, y_train['target'], X_test)
    else:
        X_test_enc = X_test

    test_preds_per_fold = np.array([
        m.predict(X_test_enc[features].fillna(0).values)
        for m in models
    ])
    test_preds = test_preds_per_fold.mean(axis=0)   # ensemble average
    ...
    submission.to_csv('preds_all_features_v2.csv')
    print("Saved preds_all_features_v2.csv")
